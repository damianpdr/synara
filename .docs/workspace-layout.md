...
- `/apps/mobile` — Expo SDK 57 iOS client (`@synara/mobile`) that runs in Expo Go. Connects to a Synara server over the network via the bearer pairing flow and speaks the Effect RPC WebSocket protocol through its own thin JSON-frame client in `apps/mobile/src/transport` (no Effect runtime in the bundle). Imports `@synara/contracts`/`@synara/shared` as types only.
