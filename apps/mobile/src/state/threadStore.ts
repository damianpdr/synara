...
import { getConnectionManager } from "@/state/synaraStore";
import {
...
export const useThreadStore = create<ThreadStoreState>((set, get) => {
  ...
  function openStream(threadId: ThreadId, lease: ThreadLease): void {
    ...
    if (leases.size > MAX_THREAD_STREAMS) {
      ...
      // somehow exhausted, drop the oldest idle one rather than let the server
      // reject the subscribe.
    ...
    lease.subscription = manager.subscribeThread(threadId, (item) => {
      ...
      set({ details: { ...details, [threadId]: applyThreadStreamItem(current, item) } });
    });
  }
  ...
  return {
    ...
    async respondToApproval({ threadId, requestId, lifecycleGeneration, decision }) {
      ...
      const ok = await dispatch(threadId, {
      ...
      // On success the card disappears when `approval.resolved` arrives; the
      // durable `responding` status keeps it disabled until then.
      if (!ok) patchRecord("responding", requestId, undefined);
      return ok;
    },
